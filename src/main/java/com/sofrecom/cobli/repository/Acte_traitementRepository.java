...
//KPI
	...
	@Query("SELECT a FROM Acte_traitement a WHERE a.typeprestation.idPrestation = :idP AND a.tarif.codeTarif = :idT")
	List<Acte_traitement> findPrestTarif(@Param("idP") int idP, @Param("idT") String idT);



    @Query("SELECT COUNT(a) FROM Acte_traitement a WHERE a.typeprestation.idPrestation = :idP")
    long findPrestTarifCount(@Param("idP") int idP);


    @Query("SELECT a FROM Acte_traitement a WHERE a.typeprestation.idPrestation = :idP ")
    List<Acte_traitement> findPrest(@Param("idP") int idP);



    @Query("SELECT a FROM Acte_traitement a WHERE a.typeprestation.idPrestation = :idP AND a.tarif.codeTarif = :idT AND a.affectation IN :cuids")
    List<Acte_traitement> findPrestTarifAndAffectationIn(@Param("idP") int idP, @Param("idT") String idT, @Param("cuids") List<String> cuids);

    List<Acte_traitement> findByAffectationIn(List<String> cuids);
